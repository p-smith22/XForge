...
void MainWindow::setupUI() {
    ...
    fAlpha->addRow("α start:", m_aStart);
    fAlpha->addRow("α end:",   m_aEnd);
    ...
    // Build all 7 charts
    buildChart(m_clView,    m_clSer,    "Lift Coefficient (2D)",   "α (deg)",    "CL",   C_CL);
    buildChart(m_cdView,    m_cdSer,    "Drag Coefficient",        "α (deg)",    "CD",   C_CD);
    buildChart(m_cmView,    m_cmSer,    "Moment Coefficient",      "α (deg)",    "CM",   C_CM);
    buildChart(m_polarView, m_polarSer, "Drag Polar",              "CD",         "CL",   C_POLAR);
    buildChart(m_ld2dView,  m_ld2dSer,  "Lift-to-Drag Ratio (2D)", "α (deg)",    "L/D",  C_LD2D);
    buildChart(m_ld3dView,  m_ld3dSer,  "Lift-to-Drag Ratio (3D)", "α (deg)",    "L/D",  C_LD3D);
    buildChart(m_cl3dView,  m_cl3dSer,  "Lift Curve (3D)",         "α 3D (deg)", "CL",   C_CL3D);

    // Tabs for each chart
    QTabWidget* tabs = new QTabWidget;
    tabs->addTab(m_clView,    "CL vs α (2D)");
    tabs->addTab(m_cl3dView,  "CL vs α (3D)");
    tabs->addTab(m_cdView,    "CD vs α");
    tabs->addTab(m_cmView,    "CM vs α");
    ...
    addRow(3, 0, "α @ CL,max",      m_lblStall,  BEIGE);
    ...
    addRow(3, 1, "α @ (L/D)max,3D", m_lblLdOpt,  BEIGE);
